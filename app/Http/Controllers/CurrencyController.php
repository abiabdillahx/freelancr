...
class CurrencyController extends Controller
{
    ...
    public function convert(Request $request, CurrencyService $currency): JsonResponse
    {
        $validated = $request->validate([
            'amount' => 'nullable|numeric|min:0',
            'base' => ['nullable', 'string', Rule::in(self::SUPPORTED_CURRENCIES)],
            'symbols' => 'nullable|string',
        ]);

        $amount = isset($validated['amount']) ? (float) $validated['amount'] : 1.0;
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data kurs: ' . $e->getMessage(),
            ], 502);
        }
    ...
    private function parseSymbols(?string $symbols, string $base): array
    {
        ...
        return collect(explode(',', $symbols))
            ...
            ->filter(fn (string $symbol) => in_array($symbol, self::SUPPORTED_CURRENCIES, true))
